...
def your_files():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "SELECT audio_id, user_id, file_name, file_type, file_size, upload_datetime FROM audiofiles WHERE user_id = %s",
        (current_user.id,),
    )
    audio_files = cursor.fetchall()  # Fetch all files for the user

    if not audio_files:
        flash("No audio files found.", "warning")

    return render_template("your_files.html", audio_files=audio_files)
