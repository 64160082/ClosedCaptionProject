...
class SentenceRecognizer:
    ...
    def _calculate_stress(self):
        ...
        for sentence in self.sentences:
            ...
            pitch_avg_sentence = audio_analyzer.get_avg_frequency(
                sentence[0][1], sentence[-1][2]
            )
            pitch_max_sentence = audio_analyzer.get_max_frequency(
                sentence[0][1], sentence[-1][2]
            )
            pitch_top10_avg_sentence = audio_analyzer.get_top10_frequency(
                sentence[0][1], sentence[-1][2]
            )
            ...
            for word_info in sentence:
                ...
                if syllable_count and syllable_count > 1:
                    ...
                    for i, sub_word in enumerate(syllables):
                        ...
                        pitch_avg = audio_analyzer.get_avg_frequency(sub_start, sub_end)
                        pitch_max = audio_analyzer.get_max_frequency(sub_start, sub_end)

                        pitch_top10_avg = audio_analyzer.get_top10_frequency(
                            sub_start, sub_end
                        )
                    ...
                    pitch_avg = audio_analyzer.get_avg_frequency(start_time, end_time)
                    pitch_max = audio_analyzer.get_max_frequency(start_time, end_time)
                    pitch_top10_avg = audio_analyzer.get_top10_frequency(
                        start_time, end_time
                    )
